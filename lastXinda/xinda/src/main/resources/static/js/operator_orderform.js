$(".search-btn").on("click", function(){
	var no=$('#no').val();//取值      val函数没加（）点击搜索一直出现400错误
    location.href="/findOperByno?no="+no;
})
$(function(){
	...
	if(cpage<=10){
		for(var i=1;i<=cpage;i++){
			if(i==nowpage){
				strhtml+='<a href=/findOperByno?pageStart='+psize*(i-1)+'&name='+name+' style="background-color:#aaaaaa">'+i+'</a>';
			}else{
				strhtml+='<span> <a href=/findOperByno?pageStart='+psize*(i-1)+'&name='+name+'>'+i+'</a></span>';
			}
		}
		...
		for(var i=1;i<=10;i++){
			if(i==nowpage){
				strhtml+='<a href=/findOperByno?pageStart='+psize*(i-1)+'&name='+name+' style="background-color:#aaaaaa;font-weight:700">'+i+'</a>';

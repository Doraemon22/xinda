...
function noComment(){
	...
	$.ajax({
		...
		success : function(data) {
			console.log("成功comment0", data);//data里存的是Controller里存在map里的值
			orderCommentList(data);//data里放了多表查询的数据，具体的页面显示函数在下面
		},
		error : function(data) {
			console.log("失败", data);
		}
...
function commented(){
	...
	$.ajax({
		...
		success : function(data) {
			console.log("成功comment0", data);//data里存的是Controller里存在map里的值
			orderCommentedList(data);//data里放了多表查询的数据，具体的页面显示函数在下面
		},
		error : function(data) {
			console.log("失败", data);
		}
...
//
function orderCommentedList(data)
{
	var eOderPro = data.eOderPro;
	var txt = "";
	$(".list").append().html("");
	if(eOderPro!=null){
		for(var i = 0;i<eOderPro.length;i++)//  <li>${eOderPro[i].sname}</li>
	...
	<img src="/commentProImgShow?pid=${eOderPro[i].id}"  alt="图片" />
            ...
            <li>${eOderPro[i].name}</li>
            <li>${eOderPro[i].info}</li>
          <li> 评价内容：${eOderPro[i].content}</li>
          
        </ul>
        <p>购买时间：${datetime(eOderPro[i].judge_time)}</p>
        <p class="evaluate_btn" >已评价</p>
			          </div><hr color="#ccc" size="1">
        `
			  if(eOderPro[i].comment_status==1){//没有办法判断
			        txt+=	``
			        }
		}
	$(".list").append(txt);}

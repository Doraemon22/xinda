...
public class ProviderController {
	...
	@RequestMapping(value = "/prologin", method = RequestMethod.POST)
		 public Map<String, Object> login(String imgcode, String cellphone, String password, HttpServletRequest request) throws NoSuchAlgorithmException, UnsupportedEncodingException {
		        MD5Util md5 = new MD5Util();
				HttpSession session = request.getSession();
		        String code = (String) session.getAttribute("code");
		        int code1 = 0;
		        System.out.println("prologin===="+password);
		 Map<String, Object> map = new HashMap<String, Object>();
	        if (imgcode.toUpperCase().equals(code) && cellphone != null) {
	         //   Provider provider = providerService.selectByPrimaryKey(providerService.getIdByCellPhone(cellphone));
	        	...
	        	Provider provider = loginList.get(0);//===
	            	if (provider.getPassword().equals(md5.EncoderByMd5(request.getParameter("password")))) {
	            	code1 = 1;
					map.put("code", code1);
...
//		}
		...
		if (imgcode.toUpperCase().equals(code)) {
			...
			if (loginList != null && loginList.size() > 0) {
				...
				if (request.getParameter("password").equals(request.getParameter("password1"))) {
					providerService.findpassword(request,provider,cellphone);
					code1 = 1;
					map.put("code", code1);
				} else {
					map.put("code", code1);
	...
	public String updateprovider(Provider provider,HttpServletRequest request) {
		...
		String regin = cmbProvince + cmbCity + cmbArea;
		provider.setRegionId(regin);
		int j=providerService.updateByProvId(provider);
